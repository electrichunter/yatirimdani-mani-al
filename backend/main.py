...
def is_market_open():
    """Borsaların açık olup olmadığını kontrol eder. Hafta sonu kapalıdır.
    Pazartesi açılıştan (01:00) 2 saat sonra (03:00) başlar.
    """
    now = datetime.now()
    weekday = now.weekday() # 0 Mon, 5 Sat, 6 Sun
    hour = now.hour
    
    # Hafta sonu kapalı (Cumartesi 01:00'den Pazartesi 03:00'e kadar)
    ...
    for t in trades:
        try:
            ...
            if t.get('notional_usd') is None:
                t['notional_usd'] = compute_notional(t.get('symbol'), t.get('lot', 0), price)
...
def compute_notional(symbol, lot, price):
    try:
        if price is None:
            return None
        # Kullanıcı Talebi: 1 Lot = 1 Adet (Unit)
        # Forex 100.000 çarpanı kaldırıldı.
        if symbol.startswith("USD"):
            # Baz döviz USD ise notional direkt lot miktarıdır (USD bazında)
            return round(lot, 2)
        
        # Diğer durumlarda (EURUSD, XAUUSD vb.) notional = lot * fiyat
...
def process_symbol(symbol, components):
    ...
    if not trade_validation["valid"]:
        ...
        signal_info = {
            ...
            "sentiment_score": stage2_result.get("sentiment_score", 0)
        }
        ui.save_result_for_web(symbol, signal_info)
        return False
    
    # Pozisyon büyüklüğü - Dinamik Hesaplama (Kullanıcı Talebi: Bakiyenin %10'u)
...
def main():
    ...
    try:
        ...
        while True:
            ...
            try:
                with open('data/market_status.json', 'w', encoding='utf-8') as f:
                    json.dump({
                        "open": market_open,
                        "message": "Borsalar kapalı (veya açılış sonrası 2 saatlik bekleme süresi). Analizler durduruldu. Bekleniyor..." if not market_open else "Piyasa Açık - İşlemler Devam Ediyor",
                        "timestamp": datetime.now().strftime("%H:%M:%S")
                    }, f, ensure_ascii=False, indent=2)
            except Exception:
                pass

            if not market_open and not force_first_run:
                logger.info("🛌 Borsalar kapalı (veya açılış sonrası 2 saatlik bekleme süresi). Analizler durduruldu. Bekleniyor...")
                time.sleep(300) # 5 dk sonra tekrar kontrol et
                continue
            
            if force_first_run and not market_open:
                logger.warning("⚠️ Borsalar kapalı ancak ilk açılış olduğu için 1 kerelik analiz yapılıyor...")
                    ...
                    def execute_position_plan(components, plan_data):
                        ...
                        for item in plan_data:
                            try:
                                ...
                                # Bakiyeden bağımsız olarak 1 lot açılması istendiği için kontrolü atlıyoruz (Simülasyon Modu)
                                spec = {
                                    'symbol': sym,
                                ...
                                t = open_simulated_trade_from_spec(spec, components)
                                if t:
                                    free_bal -= margin_req # Sadece takip amaçlı düşüyoruz
