                ...
                try:
                    ...
                    try:
                        ...
                        for sym, psize in pend:
                            try:
                                lot = float(psize) if (psize and float(psize) > 0) else 0.01
                                price = None
                                ...
                                if price:
                                    # 1 Lot = 1 Unit
                                    notional = lot
                                    if not sym.startswith("USD"):
                                        notional *= float(price)
                                    used += (notional / 100.0) # 1:100 kaldıraç
                            except Exception:
                                continue

                        stats['used_balance'] = round(used, 2)
                        # Toplam bakiyeyi sabit bir değer veya konfigürasyondan al
                        bal = float(total_balance or getattr(config, 'VIRTUAL_BALANCE', 100.0))
                        stats['free_balance'] = round(bal - used, 2)
                    except Exception:
                        pass
                    conn.close()
                except Exception:
        ...
        if parsed.path.startswith('/api/open_positions'):
            ...
            try:
                ...
                for r in rows:
                    ...
                    # Use the actual position size from DB, fallback to 0.01
                    display_lot = float(pos_size) if (pos_size and pos_size > 0) else getattr(config, 'MIN_DISPLAY_LOT', 0.01)
                    ...
                    try:
                        if entry is not None and current is not None and float(entry) > 0:
                            ...
                            # Standardize to USD (1 Lot = 1 Unit)
                            if str(symbol).startswith("USD"): # Base is USD (e.g. USDJPY)
                                notional_usd = round(display_lot, 2)
                                profit_amount = round((profit * display_lot) / current_f, 2)
                            else: # Quote is USD (e.g. EURUSD, XAUUSD)
                                notional_usd = round(display_lot * current_f, 2)
                                profit_amount = round(profit * display_lot, 2)
                        else:
